...
public class Model {
    ...
    private HashMap<Navire, ArrayList<Date>> mapNavireVsListDate;// un hashmap qui stock des navires et leurs dates
                                                                 // départ et arrival,
    ...
    public int[] positionNavire(Navire navire, Date date) {
        ...
        if (moveArrete.getDepart().equals(date))
            currentPosition = moveArrete.getFrom_id();
    ...
    public HashMap<Integer, HashSet<Navire>> getPorteAvecNavire(String date) {
        ...
        for (Entry<Navire, int[]> entry : mapNavireAvecPorte.entrySet()) {
             ...
             * if absente porteA, then create a new key porteA and associate it with a new
             * Array
    ...
    public void chargerModel(String path) throws IOException {
        ...
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            String dataOfLine[] = line.split(";");
            if (dataOfLine[2].equals("year"))
                continue; // There are 'HEADERS' inside the allMoves.csv
            /*
            ...
            if (this.setNavire.add(navireTraite)) // if navire Traité is not in the set
            {
                ...
                Move moveTraite = new Move(dataOfLine[4], dataOfLine[5], Integer.parseInt(dataOfLine[6]),
                        Integer.parseInt(dataOfLine[7]));
                ...
                if (!(this.mapNavireVsListDate.get(navireTraite).contains(dateDepart)))
                    this.mapNavireVsListDate.get(navireTraite).add(dateDepart);
                if (!(this.mapNavireVsListDate.get(navireTraite).contains(dateArrival)))
                    this.mapNavireVsListDate.get(navireTraite).add(dateArrival);
    ...
    public void afficherPostionDesNavires(String date) {
        ...
        for (Navire nav : sorted.keySet()) {
            System.out.println(String.format("%-20s", "Navire " + nav) + " :"
                    + String.format("%-20s", Arrays.toString(this.positionNavire(nav, date))));
        }
    }
